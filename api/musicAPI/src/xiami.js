 ...
 */
 ...
 const searchSong = (key, limit, page, raw, artist) => {
  if (!raw) {
    return new Promise((resolve, reject) => {
      ...
      .then(res => {
        const songList = res.data.songs.filter(item => (item.lyric && item.song_name.toLowerCase().indexOf('remix') === -1 && item.song_name.toLowerCase().indexOf('cover') === -1)).map(item => ({
          album: {
            id: item.album_id,
